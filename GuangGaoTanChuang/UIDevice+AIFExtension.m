...
@implementation UIDevice (AIFExtension)
...
- (BOOL)isIphoneX {
  ...
  if ([self isSimulator]) {
    modelIdentifier = [[NSProcessInfo processInfo] environment][@"SIMULATOR_MODEL_IDENTIFIER"];
  }else{
    modelIdentifier = [self modelIdentifier];
  }
  
  return [modelIdentifier isEqualToString:@"iPhone10,3"] || [modelIdentifier isEqualToString:@"iPhone10,6"];
}
